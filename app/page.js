...
export default function Dashboard() {
  ...
  const pendingCount    = leads.filter((l) => {
    const s = getReplyStatus(l);
    return s.label === "Pending Reply";
  }).length;
  ...
  const StatCard = ({ label, value, sub, subColor }) => (
    <div style={{
      ...
      <div style={{ fontSize: 26, fontWeight: 700, color: "#0F172A", letterSpacing: -1 }}>{value}</div>
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: "'Inter', 'Segoe UI', sans-serif", background: C.bg }}>
      ...
      <aside style={{
        ...
        padding: "24px 16px", display: "flex", flexDirection: "column", gap: 6, flexShrink: 0,
      }}>
        <div style={{
          fontSize: 16, fontWeight: 700, color: C.text,
          padding: "0 8px 20px", letterSpacing: -.3,
          borderBottom: `1px solid ${C.surface}`, marginBottom: 8,
        }}>
          Lead<span style={{ color: "#818CF8" }}>CRM</span>
        </div>
        ...
        <div style={{ marginTop: "auto", paddingTop: 16, borderTop: `1px solid ${C.surface}` }}>
          <a href="/import" style={{
            display: "flex", alignItems: "center", gap: 10,
            padding: "9px 12px", borderRadius: 8, color: C.muted,
      ...
      <main style={{ flex: 1, background: C.pageBg, overflowX: "auto" }}>
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "#0F172A", letterSpacing: -.4 }}>Lead Tracker</div>
        ...
        {importResult && (
          <div style={{ padding: "10px 28px", background: importResult.startsWith("✅") ? "#F0FDF4" : "#FEF2F2", fontSize: 13, color: importResult.startsWith("✅") ? C.green : C.red, borderBottom: "1px solid #E2E8F0" }}>
            {importResult}
          </div>
        )}
        ...
        <div style={{ padding: "0 28px 28px" }}>
          ...
          {loading ? (
            ...
            <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: "0 0 12px 12px", padding: 48, textAlign: "center", color: C.muted, fontSize: 14 }}>
              Walang leads dito.
            </div>
          ) : (
              ...
              <table style={{
                ...
                <thead>
                  <tr style={{ background: "#F8FAFC" }}>
                    {["Name","Email","Source", ...(showCommentCols ? ["Post","Comment"] : []), "Stage","Heat","Reply Status","Created","Action"]
                      .map((h) => (
                        <th key={h} style={{
                ...
                <tbody>
                  {filteredLeads().map((lead) => {
                    ...
                    const heatCfg   = lead.heat === "hot"  ? { bg: C.redBg,   color: C.red,   icon: "🔥" }
                                    : lead.heat === "cold" ? { bg: C.blueBg,  color: C.blue,  icon: "🧊" }
                                    :                        { bg: C.amberBg, color: C.amber, icon: "🔥" };
                    return (
                      <tr key={lead.id} style={{ borderBottom: "1px solid #F1F5F9" }}>
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <span
                            ...
                            style={{ fontWeight: 700, color: C.blue, cursor: lead.source === "facebook" ? "pointer" : "default" }}
                          >{lead.name}</span>
                        </td>
                        ...
                        {/* Source */}
                        <td style={{ padding: "12px 16px" }}>
                          {pill(C.blueBg, C.blue, <>📘 {lead.source}</>)}
                        </td>

                        {/* Comment cols */}
                        {showCommentCols && (
                          <td style={{ padding: "12px 16px", fontSize: 12, color: "#555", maxWidth: 160 }}>{lead.postTitle || "—"}</td>
                        )}
                        {showCommentCols && (
                          <td style={{ padding: "12px 16px", fontSize: 12, color: "#333", maxWidth: 180 }}>{lead.comment || "—"}</td>
                        )}
                        ...
                        {/* Heat */}
                        <td style={{ padding: "12px 16px" }}>
                          {pill(heatCfg.bg, heatCfg.color, <>{heatCfg.icon} {lead.heat}</>)}
                        </td>
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          {!isDone ? (
                            <button
                              onClick={() => markAsDone(lead.id, lead.source)}
                              disabled={isUpdating}
                              style={{
                                padding: "5px 12px", fontSize: 11, fontWeight: 700,
                                borderRadius: 7, border: "1.5px solid #BBF7D0",
                                background: isUpdating ? "#F1F5F9" : "#F0FDF4",
                                color: isUpdating ? C.muted : C.green,
                                cursor: isUpdating ? "not-allowed" : "pointer", minWidth: 90,
                              }}
                            >
